...
class InlineKeyboardButton(Object):
    ...
    callback_data_with_password (``bytes``, *optional*):
        A button that asks for the 2‑step verification password of the current user and then sends a callback query
        to a bot. Data to be sent to the bot via a callback query.

    copy_text (``str``, *optional*):
        A button that copies the text to the clipboard. Limited to 256 characters.

    pay (``bool``, *optional*):
        Pass True to send a Pay button. Substrings `⭐` and `XTR` in the button's text will be replaced with a
        Telegram Star icon. Available only in invoice messages.
        **NOTE**: This type of button **must** always be the first button in the first row.
    ...
    style (:obj:`~pyrogram.enums.ButtonStyle`, *optional*):
        Style of the button. Defaults to `ButtonStyle.DEFAULT`.
    """
    ...
    @staticmethod
    def read(b: "raw.base.KeyboardButton") -> "InlineKeyboardButton":
        ...
        # fallback “plain” KeyboardButton (no extra action)
    ...
    async def write(self, client: "pyrogram.Client") -> "raw.base.KeyboardButton":
        style = (
        ...
        # “plain” button (no special action)
